    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      titles.forEach(title => {
        if(title.tags ==tag){
          ...
          div.innerHTML = `<heading id='check'>${title.name}</heading><br><br><p id="check">Tags:   </p><a href="${url}tag?${title.tags}"><button id='tag'>${title.tags}</button><Br><br></a><div class="choice"><a href="${url}page?${title.name}"><button>Study</button></a><a href="${url}quiz?${title.name}"><button >Quiz</button></a></div><div id='points'  class="${name_const} clicker"><span>o</span ><span>0</span><span>o</span></div>`;
